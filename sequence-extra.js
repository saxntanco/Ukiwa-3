...
['pumps','交互運転：次はもう一台','設備','一つの出力を反転する操作とは別。','運転要求のたびにポンプA、次はBを選びます。要求OFFで停止した時点で次回担当を交代。電源断で次回をAに初期化する簡略モデルです。故障時の代替起動や増台は含みません。','要求ON → OFF → ON。担当がAからBへ交代する。','運転要求をONのままにすると？','同じ担当が運転を続ける','勝手に交代する','この例は一運転ごとに交代します。時間交代方式とは違います。'],
['alarm','警報記憶：復旧と確認は別','保護・操作','異常の消滅と、警報の解除を分ける。','異常が出たら警報ランプを記憶。原因が復旧してもランプは残し、復旧後のRESETで消します。異常が残る間はRESETしても消しません。この記憶は電源断で消えるモデルです。','異常ON → RESET → 異常OFF → RESET。','原因復旧だけでランプは？','残る','消える','一時的に発生した異常の痕跡を確認するため、復旧と解除を分けています。'],
['jog','寸動：押す間だけ動かす','保護・操作','通常運転と、短く動かす操作を比較する。','通常起動は自己保持し、寸動は押している間だけ運転します。この教育モデルでは通常運転中の寸動指令を無視します。通常運転を停止してから寸動を試してください。','通常起動 → 停止 → 寸動ON → OFF。','停止状態から寸動し離すと？','停止する','自己保持する','寸動では自己保持を成立させない条件を設けています。単にSTARTに別ボタンを並列追加するだけではありません。'],
...
const lessons=data.map(([id,title,group,goal,copy,task,q,yes,no,why],i)=>({id,title,short:title.split('：')[0],group,goal,copy:'<p>'+copy+'</p>',task,terms:'図は動作条件を理解するための機能図です。端子結線図ではありません。入力・記憶・時間を操作し、下段の接点出力を追います。タイマ値は教材用。機器ごとの復帰・再入力・停電時の扱いは取扱説明書で確認します。',q,a:i%2?[no,yes]:[yes,no],correct:i%2?1:0,why}));
function fresh(){return {power:true,input:false,other:false,permit:true,auto:false,mem:false,t:0,age:0,remaining:0,run:false,dir:0,next:0,assigned:0,history:[]};}
function output(id,s){if(!s.power)return [false,false,false];switch(id){case'momentary':return[s.input];case'alternate':case'impulse':case'latch':case'alarm':return[s.mem];case'on':return[s.input&&s.age>=3];case'off':return[s.input||s.remaining>0];case'interval':return[s.input&&s.age<3];case'pulse':return[s.remaining>0];case'flicker':return[s.input&&Math.floor(s.age/2)%2===1];case'sequence':return[s.run,s.run&&s.other];case'reverse':return[s.dir===1,s.dir===2];case'star':return[s.run,s.run&&s.age<3,s.run&&s.age>=4];case'pumps':return[s.input&&s.assigned===0,s.input&&s.assigned===1];case'jog':return[s.run||s.input];case'auto':return[s.permit&&(s.auto?s.other:s.input)];}return[false];}
function update(id,s,key){const was=s.input; if(key==='power'){s.power=!s.power;if(!s.power){s.mem=id==='alternate'?s.mem:false;s.remaining=0;s.age=0;s.run=false;s.dir=0;s.input=false;s.other=false;s.next=0;s.assigned=0;}}else if(key==='tick'){s.t++;if(s.power){if(s.input||s.run)s.age++;if(s.remaining>0)s.remaining--;}}else if(s.power){if(key==='input')s.input=!s.input;if(key==='other')s.other=!s.other;if(key==='permit')s.permit=!s.permit;if(key==='auto')s.auto=!s.auto;if(key==='start'&&!s.run){s.run=true;s.age=0;}if(key==='stop'){s.run=false;s.dir=0;s.input=false;s.age=0;}if(key==='forward'&&s.dir===0)s.dir=1;if(key==='reverse'&&s.dir===0)s.dir=2;if(id==='alarm'&&key==='clear'&&!s.input)s.mem=false;
 if(id==='alternate'&&key==='input'&&!was&&s.input)s.mem=!s.mem;
...
why=!s.power?'電源OFF。出力はすべてOFF。'+(id==='alternate'?'機械的な接点位置は保持。':'このモデルの運転記憶・計時はリセット。'):rules[id]+'。現在：'+out.map((x,i)=>labels[i]+' '+(x?'ON':'OFF')).join(' ／ ');
...
const rows=out.map((on,i)=>`<div class="functional-row ${on?'running':''}"><span>電源</span><b>→</b><span>出力接点<br>${(id==='alternate'?s.mem:on)?'閉':'開'}</span><b>→</b><span>${labels[i]}<br><strong>${on?'ON':'OFF'}</strong></span><b>→</b><span>戻り</span></div>`).join('');
return {why,html:`<div class="function-diagram"><p class="caption">機能図：${rules[id]}<br>内部論理を省略した出力側の模式図です。</p><pre class="circuit-recipe">${circuits[id]}</pre><div class="logic-box"><b>入力・記憶・時間</b><p>入力 ${s.input?'ON':'OFF'} ／ 第2入力 ${s.other?'ON':'OFF'}<br>記憶 ${s.mem?'ON':'OFF'} ／ 模擬時刻 ${s.t}秒</p></div><p class="caption">↓ 条件を判定して出力接点を動かす</p>${rows}</div>`,history:s.history};}
