...
import { bgm, music, sound, effects, toggleEffects } from './audio.js';
...
let deckId = 'balanced', enemyDeck = 'balanced', musicError = '';
...
function start() { cancelTimers(); modal.close(); enemyDeck = ['rush', 'guard', 'storm'][Math.floor(Math.random() * 3)]; game = newGame(Math.random, [DECKS[deckId].cards, DECKS[enemyDeck].cards]); sound('summon'); screen = 'battle'; locked = game.active === 1; selection = null; notice = `${game.first === 0 ? 'あなた' : 'CPU'}が先攻です。`; cpuSteps = 0; render(); if (game.active === 1)
    later(cpu, 950); }
...
function audioControls() { return `<div class="audio-controls"><button data-music aria-pressed="${!bgm.paused}">${bgm.paused ? '▶ BGM' : 'Ⅱ BGM'}</button><label>音量<input data-volume aria-label="BGM音量" type="range" min="0" max="100" value="${Math.round(bgm.volume * 100)}"></label><button data-effects aria-pressed="${effects}">効果音 ${effects ? 'ON' : 'OFF'}</button>${musicError ? '<span role="status">' + musicError + '</span>' : ''}</div>`; }
function render() {
    if (screen === 'title') {
        app.innerHTML = `<header class="sitebar"><a href="../index.html">← うきわメモ</a><span>UKIWA ARCADE / 001</span><a href="../hidden-menu.html">秘密のラウンジ</a></header><main class="title-screen"><div class="title-copy"><span class="eyebrow">A DUEL WRITTEN IN ELECTRICITY</span><h1>電界決闘<span>CIRCUIT DUEL</span></h1><p class="tagline">小さな火花が、<br>世界をひっくり返す。</p><p class="intro">電気の精霊を呼び、回路を制する。<br>10種類のカードで挑む、あなたとCPUの決闘。</p>${audioControls()}<div class="deck-picker" aria-label="デッキ選択">${Object.keys(DECKS).map(id => `<button data-deck="${id}" aria-pressed="${id === deckId}"><b>${DECKS[id].name}</b><small>${DECKS[id].tip}</small></button>`).join('')}</div><div class="title-actions"><button class="primary start" data-start>対戦開始 <span>↗</span></button><button data-how>遊び方・カード</button></div><div class="title-meta"><span>20<small>ライフ</small></span><span>10<small>カード種</small></span><span>5–10<small>分の決闘</small></span></div></div><div class="showcase" aria-label="電気の精霊たち"><div class="orbital orbital-one"></div><div class="orbital orbital-two"></div>${['resistor', 'tesla', 'coil'].map((id, i) => `<button class="show-card show-${i}" data-info="${id}" style="--accent:${CARDS[id].color}" aria-label="${CARDS[id].name}の詳細">${cardFace({ uid: 'preview-' + id, card: id })}<p class="flavor">${CARDS[id].flavor}</p></button>`).join('')}<span class="showcase-caption">RESIST. AMPLIFY. STRIKE.</span></div><footer class="title-footer"><span>構築済み20枚を選択。CPUも3つの戦術から選びます。</span><span>BGM: <a href="https://incompetech.com/music/royalty-free/index.html?isrc=USUAN1100844" target="_blank" rel="noopener">Cipher — Kevin MacLeod</a> / <a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noopener">CC BY 4.0</a>（音源改変なし）</span></footer></main>`;
        return;
    }
    if (!game)
        return;
...
function commit(action, side) {
    ...
    game = r.state;
    sound(game.winner !== null ? 'result' : action.type === 'attack' ? 'attack' : action.type === 'play' ? 'spell' : 'turn');
...
function handle(e) {
    ...
    if ('music' in b) {
        musicError = '';
        void music().then(() => render()).catch((error) => { musicError = error.name === 'NotAllowedError' ? '再生がブラウザーに制限されています。外部ブラウザーで開いてください。' : '音源を読み込めません。再試行できます。'; console.warn('BGM', error.name, error.message); render(); });
        return;
    }
    if ('effects' in b) {
        toggleEffects();
    ...
    if (b.deck && b.deck in DECKS && screen === 'title') {
        deckId = b.deck;
        render();
        return;
    }
...
app.addEventListener('input', e => { const t = e.target; if (t.hasAttribute('data-volume'))
    bgm.volume = Number(t.value) / 100; });
